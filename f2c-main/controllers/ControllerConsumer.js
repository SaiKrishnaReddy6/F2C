...
const {getSignupModel} = require("../models/ModelSignup");
const Signup = getSignupModel();
...
function doSaveSignup(req, resp) {
    const info = new Signup(req.body);
    console.log(info);
    info.save().then((ans) => {
        console.log(ans);
        // resp.send(ans);
...
module.exports = { doSaveProduct, doDeleteProduct, showAll, findd, doUpdate, doSave, doSaveSignup, doLogin, doAddItem, doFindItem, doFindCity, doFindGrower }